...
# so every rebuild picks up whatever's currently tagged `latest` on npm.
#
...
# build so the first agent wake doesn't have to apt-install.
...
# To pin to a specific release, change `agentdm@latest` to `agentdm@<version>`.
